#!/usr/bin/env python3
"""
Experiment 05: Refined Prompts Test - QWEN3 14B ONLY
Testing clearer, humor-focused scenarios at ~50-60 words

Model: Qwen3 14B Instruct (Local via LM Studio - MUST BE LOADED FIRST)
Note: This is a THINKING MODEL - produces <think> tags with reasoning traces

Prerequisites:
1. LM Studio running at http://localhost:1234
2. Load qwen3-14b-instruct model (Q4_K_M quantization) in LM Studio
3. Run this script

Output: Produces thinking traces in <think> tags. Actual output must be extracted.
"""
...
def test_qwen(prompt: str, scenario_name: str) -> Dict:
    """Test Qwen3 14B via LM Studio"""
    print(f"\n🤖 Testing Qwen3 14B - {scenario_name}...")
    ...
    try:
        response = requests.post(
            ...
            json={
                ...
                "temperature": 0.7,
                "max_tokens": 1200  # Thinking model needs more tokens (thinking + output)
            },
            timeout=120
        )
        ...
        result = {
            ...
            "note": "Thinking model - actual output extracted after </think> tag"
        }
...
def print_result(result: Dict):
    ...
    print("\n" + "-" * 70)
    print("FULL RESPONSE (including <think> tags):")
    print("-" * 70)
...
def main():
    """Run Experiment 05: Qwen3 14B Test"""
    print("=" * 70)
    print("EXPERIMENT 05: REFINED PROMPTS TEST - QWEN3 14B (THINKING MODEL)")
    print("=" * 70)
    print("\nPrerequisite: LM Studio running with qwen3-14b-instruct loaded")
    print("Testing clearer, humor-focused scenarios at ~50-60 words\n")
    print("Note: This model produces <think> tags. Actual output is extracted.\n")
    ...
    for scenario_name, prompt in scenarios:
        ...
        result = test_qwen(prompt, scenario_name)
        if "error" in result:
            print(f"❌ Test failed: {result['error']}")
            print("\n⚠️  Make sure:")
            print("   1. LM Studio is running at http://localhost:1234")
            print("   2. qwen3-14b-instruct model is loaded")
            continue

        results.append(result)
        print_result(result)
        save_result(result, f"05_qwen_{scenario_name.lower().replace(' ', '_').replace('-', '')}_{timestamp}.json")
