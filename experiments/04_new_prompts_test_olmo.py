#!/usr/bin/env python3
"""
Experiment 04: New Prompts Validation Test - OLMO 3 32B ONLY
Testing short, relatable scenarios with OLMo 3 32B (local OSS model)
...
Model (this script):
- OLMo 3 32B Think (Local via LM Studio - MUST BE LOADED)

Note: API models (GPT-5.2, Claude, Gemini) already tested in 04_new_prompts_test_frontier.py
Note: Run 04_new_prompts_test_llama.py separately with Llama 3.1 8B loaded in LM Studio
"""
    ...
    try:
        response = requests.post(
            ...
            json={
                ...
                "messages": [{"role": "user", "content": prompt}],
                "temperature": 0.7,
                "max_tokens": 2000  # Increased to allow extensive thinking + output
            },
            timeout=120
        )
        ...
        result = {
            ...
            "prompt": prompt,
            "temperature": 0.7,
            "max_tokens": 2000,  # Actual value used for extensive thinking models
            "response": content,
...
def main():
    ...
    for scenario_name, prompt in scenarios:
        ...
        # API models already tested in 04_new_prompts_test_frontier.py
        # Only testing OLMo 3 32B in this script

        # OLMo 3 32B Think (local) - run this script with OLMo loaded
        result = test_local_model(prompt, scenario_name, "OLMo 3 32B Think", "allenai/olmo-3-32b-think")
        if "error" not in result:
            results.append(result)
            print_result(result)
            save_result(result, f"04_olmo3_{scenario_name.lower().replace(' ', '_').replace('-', '')}_{timestamp}.json")
