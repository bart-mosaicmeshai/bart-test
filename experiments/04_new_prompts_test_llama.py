#!/usr/bin/env python3
"""
Experiment 04: New Prompts Validation Test - LLAMA 3.1 8B ONLY
Testing short, relatable scenarios with Llama 3.1 8B (local OSS model)
...
Model (this script):
- Llama 3.1 8B Instruct (Local via LM Studio - MUST BE LOADED)

Note: API models (GPT-5.2, Claude, Gemini) already tested in 04_new_prompts_test_frontier.py
Note: Run 04_new_prompts_test_olmo.py separately with OLMo 3 loaded in LM Studio
"""
...
def main():
    ...
    for scenario_name, prompt in scenarios:
        ...
        # Only testing Llama 3.1 8B in this script

        # Llama 3.1 8B Instruct (local) - run this script with Llama loaded
        result = test_local_model(prompt, scenario_name, "Llama 3.1 8B Instruct", "meta-llama-3.1-8b-instruct")
        if "error" not in result:
            results.append(result)
            print_result(result)
            save_result(result, f"04_llama_{scenario_name.lower().replace(' ', '_').replace('-', '')}_{timestamp}.json")
