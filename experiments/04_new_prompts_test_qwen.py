#!/usr/bin/env python3
"""
Experiment 04: New Prompts Validation Test - QWEN3 14B ONLY
Testing short, relatable scenarios with Qwen3 14B (local OSS model)
...
Model (this script):
- Qwen3 14B Instruct (Local via LM Studio - MUST BE LOADED)

Note: API models (GPT-5.2, Claude, Gemini) already tested in 04_new_prompts_test_frontier.py
Note: Run other local model scripts separately
"""
    ...
    try:
        response = requests.post(
            ...
            json={
                ...
                "messages": [{"role": "user", "content": prompt}],
                "temperature": 0.7,
                "max_tokens": 1000  # Increased to allow thinking + output
            },
            timeout=120
        )
        ...
        result = {
            ...
            "prompt": prompt,
            "temperature": 0.7,
            "max_tokens": 1000,  # Actual value used for thinking models
            "response": content,
...
def main():
    ...
    for scenario_name, prompt in scenarios:
        ...
        # Only testing Qwen3 14B in this script

        # Qwen3 14B Instruct (local) - run this script with Qwen loaded
        result = test_local_model(prompt, scenario_name, "Qwen3 14B Instruct", "qwen3-14b-instruct")
        if "error" not in result:
            results.append(result)
            print_result(result)
            save_result(result, f"04_qwen_{scenario_name.lower().replace(' ', '_').replace('-', '')}_{timestamp}.json")
